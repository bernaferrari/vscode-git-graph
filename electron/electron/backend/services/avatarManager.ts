...
export class AvatarManager extends Disposable {
	...
	public async getAvatarImage(email: string): Promise<string | null> {
		if (!this.avatars[email] || !this.avatars[email].image) {
			return null;
		}

		return new Promise((resolve) => {
			const imagePath = path.join(this.avatarStorageFolder, this.avatars[email].image);
			fs.readFile(imagePath, (err, data) => {
				if (err) {
					resolve(null);
				} else {
					const format = this.avatars[email].image.split('.')[1];
	 ...
	 */
	private getRemoteSource(remoteUrl: string): { type: 'github' | 'gitlab' | 'gravatar'; owner?: string; repo?: string } {
		// GitHub HTTPS
			...
			https.get({
				...
				res.on('end', async () => {
					try {
						...
						if (Array.isArray(commits) && commits.length > 0 && commits[0].author?.avatar_url) {
							const avatarUrl = commits[0].author.avatar_url + '&size=162';
				...
				res.on('end', async () => {
					try {
						...
						if (result.avatar_url) {
							const img = await this.downloadAvatarImage(item.email, result.avatar_url);
							if (img) {
								this.saveAvatar(item.email, img, false);
								resolve(true);
								return;
