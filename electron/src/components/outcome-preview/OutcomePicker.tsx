...
const SINGLE_COPY: Record<
	IntegrationStrategy,
	{ title: string; tagline: string; explainer: string; commits: string }
> = {
	merge: {
		title: 'Preserve the branch',
		tagline: 'You see exactly what happened',
		explainer: 'Both histories stay intact. A merge commit ties them together so you can always trace where work came from.',
		commits: 'Adds 1 merge commit',
	},
	rebase: {
		title: 'Make a clean line',
		tagline: 'Looks like you wrote it on top',
		explainer: "Your commits are replayed on top of the target. The history reads as a single line, but commit hashes change.",
		commits: 'Replays your commits',
	},
	squash: {
		title: 'One tidy commit',
		tagline: 'A single change to review',
		explainer: 'Everything you did becomes one commit. Best for tight pull requests where reviewers only want the final shape.',
...
const BATCH_COPY: Record<
	BatchStrategy,
	{ title: string; tagline: string; explainer: (n: number) => string; commits: (n: number) => string }
> = {
	'cherry-pick': {
		title: 'Bring these in',
		tagline: 'Copy onto current branch',
		explainer: (n) => `Each of the ${String(n)} selected commits is replayed on top of the current branch. Hashes change but messages and authors are preserved.`,
		commits: (n) => `Adds ${String(n)} commit${n === 1 ? '' : 's'}`,
	},
	'squash-batch': {
		title: 'Squash into one',
		tagline: 'Combine selected into a single commit',
		explainer: (n) => `The ${String(n)} selected commits collapse into one. Best when they form a single logical change. Requires the commits to be contiguous on the current branch.`,
		commits: () => 'Collapses to 1 commit',
	},
	drop: {
		title: 'Drop from history',
		tagline: 'Remove these commits entirely',
		explainer: (n) => `History is rewritten to skip over the ${String(n)} selected commits. The changes those commits introduced are removed.`,
				...
				<div className='grid grid-cols-1 gap-3 p-5 md:grid-cols-3'>
					{orderedOptions.map((option) => (
						<OutcomeCard
							...
							onSelect={() => { setSelected(option.strategy); }}
						/>
					))}
				</div>
			...
			<DialogContent className='max-w-4xl gap-0 overflow-hidden p-0'>
				<DialogHeader className='space-y-1 border-b border-border/60 px-5 py-4'>
					<DialogTitle className='flex flex-wrap items-center gap-2 text-[0.9375rem]'>
						...
						<span className='font-semibold'>{count} commit{count === 1 ? '' : 's'} selected</span>
						<span className='font-mono text-[0.8125rem] text-muted-foreground/85'>
							→ {targetRef}
						</span>
					</DialogTitle>
				...
				<PickerFooter
					...
					onCancel={() => { onOpenChange(false); }}
					onApply={() => { onApply(selected, orderedCommits); }}
				/>
			</DialogContent>
		</Dialog>
	);
...
function CommitChips({
	...
	return (
		<div className='flex flex-wrap items-center gap-1.5 border-b border-border/60 bg-muted/15 px-5 py-2.5'>
			<span className={cn(SECTION_LABEL, 'mr-0.5')}>
				{reorderable ? 'Order' : 'Selected'}
			</span>
			{visible.map((c, i) => (
				<CommitChip
					key={c.hash}
			...
			{overflow > 0 && (
				<span className='inline-flex items-center rounded-md border border-border/70 bg-card/60 px-1.5 py-0.5 font-mono text-[10px] tabular-nums text-muted-foreground/85'>
					+{overflow}
				</span>
			)}
			{reorderable && (
	...
	return (
		<>
			{insertingBefore && (
				<span
					aria-hidden
					className='inline-block h-5 w-[2px] rounded-full bg-primary shadow-[0_0_0_2px_color-mix(in_oklch,var(--primary)_20%,transparent)]'
				/>
			)}
			<span
				...
				{reorderable && (
					<>
						<GripVertical className='h-2.5 w-2.5 shrink-0 text-muted-foreground/60' aria-hidden />
						<span className='inline-grid h-3.5 min-w-3.5 place-items-center rounded-sm bg-primary/15 px-1 text-[9px] font-semibold text-primary'>
							{index + 1}
						</span>
					</>
				)}
				<span>{commit.hash.slice(0, 7)}</span>
				<span className='truncate font-sans text-[10px] text-muted-foreground/85'>
					{subject}
				</span>
			</span>
		</>
...
function OutcomeCard({
	...
	return (
		<button
			...
			{!disabled && selected && (
				<span aria-hidden className='absolute inset-y-3 left-0 w-[2px] rounded-r-full bg-primary' />
			)}
			{recommended && !disabled && (
				<span className='absolute right-3 top-3 inline-flex items-center gap-1 rounded-full bg-primary/12 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-[0.06em] text-primary ring-1 ring-primary/20'>
			...
			<div className='space-y-0.5'>
				...
				<h3 className='text-[0.9375rem] font-semibold leading-tight'>{copy.title}</h3>
			</div>

			<div className='-mx-3.5 my-3 border-t border-border/60' />
			...
			<dl className='mt-3 space-y-1 text-[11px]'>
				...
				<Detail
					label='Conflicts'
					value={conflicts === 0 ? 'None detected' : `${String(conflicts)} file${conflicts === 1 ? '' : 's'}`}
					tone={hasConflicts ? 'destructive' : 'success'}
					icon={hasConflicts ? AlertTriangle : CheckCircle2}
				/>
			</dl>

			{disabled && disabledReason && (
				<p className='mt-2 rounded-md border border-border/60 bg-muted/40 px-2 py-1.5 text-[10px] leading-snug text-muted-foreground/85'>
					{disabledReason}
				</p>
			)}
		</button>
			...
			<svg viewBox='0 0 200 80' role='img' aria-hidden className='h-[80px] w-full'>
				{variant === 'merge' && (
					<>
						...
						<path d='M70 40 C90 40 90 14 110 14 L150 14 C170 14 170 40 180 40' stroke={lane2} strokeWidth='1.5' fill='none' />
